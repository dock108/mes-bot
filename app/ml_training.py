...
class ModelTrainer:
    ...
    async def train_entry_model(self, 
                              start_date: date,
                              end_date: date,
                              algorithm: str = 'random_forest') -> Optional[EntryPredictionModel]:
    ...
    async def train_exit_model(self,
                             start_date: date,
                             end_date: date,
                             algorithm: str = 'random_forest') -> Optional[ExitPredictionModel]:
    ...
    async def train_strike_model(self,
                               start_date: date,
                               end_date: date,
                               algorithm: str = 'random_forest') -> Optional[StrikeOptimizationModel]:
    ...
    async def hyperparameter_optimization(self,
        ...
        try:
            ...
            # Create model
            if model_type == 'entry':
                base_model = EntryPredictionModel(TrainingConfig(
                ...
                base_model = ExitPredictionModel(TrainingConfig(
                    model_type=model_type,
                    algorithm=algorithm,
                    hyperparameters={},
                    train_start_date=start_date,
                ...
                base_model = StrikeOptimizationModel(TrainingConfig(
                    model_type=model_type,
                    algorithm=algorithm,
                    hyperparameters={},
                    train_start_date=start_date,
    ...
    async def _save_model_metadata(self,
        ...
        try:
            ...
            existing_model = session.query(MLModelMetadata).filter(
    ...
    async def check_and_retrain_models(self):
        ...
        try:
            for model_type, interval in self.retraining_schedule.items():
                # Get latest model
                latest_model = session.query(MLModelMetadata).filter(
                    MLModelMetadata.model_type == model_type,
                    MLModelMetadata.is_active == True
                ).order_by(MLModelMetadata.trained_on.desc()).first()
                ...
                if not latest_model:
                    ...
                    if days_since_training >= interval.days:
                        logger.info(f"{model_type} model is {days_since_training} days old, retraining")
                        should_retrain = True
