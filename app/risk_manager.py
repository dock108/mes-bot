...
class RiskManager:
    ...
    def can_open_new_trade(self, premium_cost: float, current_equity: float) -> Tuple[bool, str]:
        ...
        if open_trades_count >= config.trading.max_open_trades:
            return (
                False,
                f"Maximum open trades limit reached ({open_trades_count}/{config.trading.max_open_trades})",
            )
        ...
        if premium_cost > config.trading.max_premium_per_strangle:
            return (
                False,
                f"Premium cost ${premium_cost:.2f} exceeds limit ${config.trading.max_premium_per_strangle}",
            )
        ...
        if current_equity < required_cash:
            return (
                False,
                f"Insufficient funds: need ${required_cash:.2f}, have ${current_equity:.2f}",
            )
        ...
        if self.daily_start_equity:
            ...
            if current_drawdown > config.trading.max_drawdown:
                return (
                    False,
                    f"Maximum drawdown exceeded: ${current_drawdown:.2f} > ${config.trading.max_drawdown}",
                )
        ...
        if not self._check_minimum_time_between_trades():
            return (
                False,
                f"Minimum time between trades not met ({config.trading.min_time_between_trades} minutes)",
            )
    ...
    def _get_consecutive_losses(self) -> int:
        ...
        try:
            ...
            recent_trades = (
                session.query(Trade)
                .filter(Trade.status.in_(["CLOSED_LOSS", "EXPIRED", "CLOSED_WIN"]))
    ...
    def get_position_sizing_recommendation(
        self, available_equity: float, option_premium: float
    ) -> int:
    ...
    def should_close_all_positions(
        self, current_equity: float, current_time: datetime
    ) -> Tuple[bool, str]:
    ...
    def update_daily_summary(self):
        ...
        try:
            ...
            daily_summary = session.query(DailySummary).filter(DailySummary.date == today).first()
            ...
            gross_profit = (
                sum(t.realized_pnl for t in today_trades if t.realized_pnl and t.realized_pnl > 0)
                or 0
            )
            gross_loss = (
                sum(t.realized_pnl for t in today_trades if t.realized_pnl and t.realized_pnl < 0)
            ...
            daily_summary.max_concurrent_trades = max(
                self._get_max_concurrent_trades_today(), daily_summary.max_concurrent_trades or 0
            )
