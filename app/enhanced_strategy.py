...
class EnhancedLottoGridStrategy(LottoGridStrategy):
    ...
    async def place_enhanced_strangle_trade(self,
                                          call_strike: float,
                                          put_strike: float,
                                          position_multiplier: float = 1.0,
                                          profit_target_multiplier: float = 4.0,
                                          ml_signal: Optional[TradingSignal] = None) -> Optional[Dict]:
        """Place strangle with enhanced ML-based parameters"""
        try:
            logger.info(f"Placing enhanced strangle: {call_strike}C / {put_strike}P")
            logger.info(f"Position multiplier: {position_multiplier:.2f}, Profit target: {profit_target_multiplier:.1f}x")
            ...
            # Enhanced risk check
            can_trade, reason = self.risk_manager.can_open_new_trade(adjusted_premium, current_equity)
            if not can_trade:
            ...
            strangle_result = await self.ib_client.place_strangle(
                ...
                put_strike,
                expiry,
                config.trading.max_premium_per_strangle * position_multiplier  # Adjusted max premium
            )
    ...
    async def _collect_market_features(self):
        ...
        try:
            ...
            if (self.last_feature_collection and 
                (current_time - self.last_feature_collection).total_seconds() < 300):
                return
    ...
    def _combine_signals(self, 
                        basic_should_trade: bool, 
                        basic_reason: str, 
                        ml_signal: TradingSignal) -> Tuple[bool, str]:
        ...
        if ml_signal.confidence > 0.8:
            ...
            reason = f"High ML confidence ({ml_signal.confidence:.2f}): {', '.join(ml_signal.reasoning)}"
    ...
    async def _record_decision(self, ml_signal: TradingSignal):
        ...
        try:
            decision = DecisionHistory(
                ...
                suggested_call_strike=ml_signal.optimal_strikes[0] if ml_signal.optimal_strikes else None,
                suggested_put_strike=ml_signal.optimal_strikes[1] if ml_signal.optimal_strikes else None,
            ...
            logger.debug(f"Recorded decision: {ml_signal.action} with confidence {ml_signal.confidence:.2f}")
    ...
    async def _record_enhanced_trade(self,
                                   strangle_result: Dict,
                                   position_multiplier: float,
                                   profit_target_multiplier: float,
                                   ml_signal: Optional[TradingSignal]) -> Trade:
    ...
    async def _link_decision_to_trade(self, trade_id: int, ml_signal: TradingSignal):
        ...
        try:
            ...
            recent_decision = session.query(DecisionHistory).filter(
            ...
            ).order_by(DecisionHistory.timestamp.desc()).first()
    ...
    async def _cleanup_old_data(self):
        ...
        try:
            ...
            deleted_decisions = session.query(DecisionHistory).filter(
                DecisionHistory.timestamp < cutoff_date_decisions
            ).delete()
            
            session.commit()
            
            logger.info(f"Cleaned up {deleted_market_data} old market data records and {deleted_decisions} old decision records")
