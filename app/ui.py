...
class Dashboard:
    ...
    def load_daily_summary(self) -> Dict:
        ...
        try:
            ...
            win_rate = summary.winning_trades / summary.total_trades if summary.total_trades > 0 else 0
        ...
        try:
            ...
            for trade in trades:
                data.append({
                    ...
                    'Return %': f"{(trade.realized_pnl / trade.total_premium * 100):.1f}%" if trade.total_premium > 0 else "0%",
                    'Status': trade.status
                })
        ...
        try:
            ...
            for summary in summaries:
                ...
                data.append({
                    ...
                    'Win Rate': summary.winning_trades / summary.total_trades if summary.total_trades > 0 else 0
                })
    ...
    def render_live_monitoring(self):
        ...
        with col5:
            st.metric(
                ...
                delta=f"-{daily_summary['losing_trades']}" if daily_summary['losing_trades'] > 0 else None
            )
        ...
        if not recent_trades_df.empty:
            ...
            styled_df = recent_trades_df.style.applymap(
    ...
    def render_backtesting(self):
        ...
        if run_backtest:
            ...
            if days_back > 59:
                st.warning(f"⚠️ Yahoo Finance only provides intraday data for the last 60 days. Adjusting start date.")
            ...
            try:
                ...
                backtest_name = st.text_input("Save backtest as:", value=f"Backtest_{datetime.now().strftime('%Y%m%d_%H%M')}")
        ...
        session = self.get_session()
        try:
            saved_backtests = session.query(BacktestResult).order_by(
    ...
    def render_configuration(self):
        ...
        with col1:
            ...
            st.write(f"**Min Time Between Trades:** {config.trading.min_time_between_trades} minutes")
        ...
        with col2:
            ...
            st.write(f"**Market Open:** {config.market_hours.market_open_hour:02d}:{config.market_hours.market_open_minute:02d} ET")
            st.write(f"**Market Close:** {config.market_hours.market_close_hour:02d}:{config.market_hours.market_close_minute:02d} ET")
            st.write(f"**Flatten Time:** {config.market_hours.flatten_hour:02d}:{config.market_hours.flatten_minute:02d} ET")
...
def main():
    ...
    tab = st.sidebar.radio(
        "Select View",
        ["📊 Live Monitor", "📈 Performance", "🔄 Backtesting", "⚙️ Configuration"]
    )
