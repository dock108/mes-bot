...
class ParameterOptimizer:
    ...
    def _get_default_parameter_ranges(self) -> Dict[str, ParameterRange]:
        ...
        return {
            ...
            "implied_move_multiplier_1": ParameterRange(
                name="implied_move_multiplier_1", min_value=1.0, max_value=2.0, step=0.1
            ),
            "implied_move_multiplier_2": ParameterRange(
                name="implied_move_multiplier_2", min_value=1.2, max_value=2.5, step=0.1
            ),
            "volatility_threshold": ParameterRange(
                name="volatility_threshold", min_value=0.5, max_value=0.9, step=0.05
            ),
            "profit_target_multiplier": ParameterRange(
                name="profit_target_multiplier", min_value=2.0, max_value=6.0, step=0.5
            ),
            ...
            "max_drawdown": ParameterRange(
                name="max_drawdown", min_value=500, max_value=1500, step=100
            ),
            "max_premium_per_strangle": ParameterRange(
                name="max_premium_per_strangle", min_value=15, max_value=40, step=5
            ),
            ...
            "ml_confidence_threshold": ParameterRange(
                name="ml_confidence_threshold", min_value=0.4, max_value=0.8, step=0.05
            ),
            "ensemble_weight_ml": ParameterRange(
                name="ensemble_weight_ml", min_value=0.1, max_value=0.7, step=0.1
            ),
        }
    ...
    async def grid_search_optimization(
        ...
        if len(all_combinations) > max_combinations:
            ...
            combinations = np.random.choice(
                all_combinations, size=max_combinations, replace=False
            ).tolist()
        else:
        ...
        results = await self._run_parallel_backtests(combinations, start_date, end_date)
        ...
        # Save results
        result_file = (
            self.results_dir / f"grid_search_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pkl"
        )
    ...
    async def bayesian_optimization(
        ...
        start_time = datetime.now()
        initial_results = await self._run_parallel_backtests(initial_points, start_date, end_date)
                    ...
                    if param_range.parameter_type == "discrete":
                        ...
                        if best_params and param_name in best_params:
                            ...
                            noise = np.random.normal(
                                0, 0.1 * (param_range.max_value - param_range.min_value)
                            )
                            value = np.clip(
                                center + noise, param_range.min_value, param_range.max_value
                            )
                        else:
    ...
    def _get_cache_key(
        self, params: Dict[str, Any], start_date: Optional[datetime], end_date: Optional[datetime]
    ) -> str:
    ...
    def analyze_optimization_results(self, result: OptimizationResult) -> Dict[str, Any]:
        ...
        for param_name in result.best_parameters.keys():
            ...
            correlation = (
                np.corrcoef(param_values, scores)[0, 1] if len(set(param_values)) > 1 else 0
            )
    ...
    def _generate_recommendations(self, result: OptimizationResult) -> List[str]:
        ...
        if result.best_score > 0.7:
            recommendations.append(
                "Optimization found good parameters - consider using these settings"
            )
        elif result.best_score > 0.5:
            recommendations.append(
                "Moderate improvement found - may need more iterations or different parameter ranges"
            )
        else:
            recommendations.append(
                "Limited improvement - consider expanding parameter ranges or checking data quality"
            )

        # Check optimization time
        if result.optimization_time > 3600:  # 1 hour
            recommendations.append(
                "Optimization took significant time - consider reducing parameter ranges or using fewer iterations"
            )

        return recommendations

    def save_optimization_report(
        self, result: OptimizationResult, analysis: Dict[str, Any], filename: str
    ):
    ...
    **kwargs,
    ...
    if method == "grid_search":
        result = await optimizer.grid_search_optimization(
            parameter_ranges=parameter_ranges, start_date=start_date, end_date=end_date, **kwargs
        )
