...
from app.models import (
    MarketData, MarketFeatures as MarketFeaturesModel, DecisionHistory,
    MLPrediction, PerformanceMetrics, get_session_maker, Trade
)
...
class FeatureCollector:
    ...
    def collect_market_data_sync(self, 
                               price: float,
                               bid: float, 
                               ask: float,
                               volume: float,
    ...
    async def calculate_and_store_features(self, 
                                         current_price: float,
                                         implied_move: float,
                                         vix_level: Optional[float] = None,
...
class FeatureEngineer:
    ...
    async def calculate_and_store_features(self, current_data, historical_data, 
                                         timestamp: Optional[datetime] = None) -> Optional[int]:
    ...
    def get_training_data(self, 
                         start_date: date,
                         end_date: date,
                         target_type: str = 'entry') -> pd.DataFrame:
        ...
        try:
            ...
            query = session.query(
                ...
                DecisionHistory, 
                MarketFeaturesModel.id == DecisionHistory.features_id
            ).filter(
                and_(
            ...
            for features, decision in results:
                ...
                if target_type == 'entry':
                    ...
                    row['target_win_probability'] = self._calculate_win_probability(decision.actual_outcome)
                elif target_type == 'exit':
    ...
    def create_features_for_prediction(self, features: MarketFeatures) -> np.ndarray:
        ...
        feature_order = [
            'realized_vol_15m', 'realized_vol_30m', 'realized_vol_60m', 'realized_vol_2h',
            'realized_vol_daily', 'atm_iv', 'iv_rank', 'iv_percentile', 'iv_skew',
            'iv_term_structure', 'rsi_15m', 'rsi_30m', 'macd_signal', 'macd_histogram',
            'bb_position', 'bb_squeeze', 'price_momentum_15m', 'price_momentum_30m',
            'price_momentum_60m', 'support_resistance_strength', 'mean_reversion_signal',
            'bid_ask_spread', 'option_volume_ratio', 'put_call_ratio', 'gamma_exposure',
            'vix_level', 'vix_term_structure', 'market_correlation', 'volume_profile',
            'time_of_day', 'day_of_week', 'time_to_expiry', 'days_since_last_trade',
            'win_rate_recent', 'profit_factor_recent', 'sharpe_ratio_recent'
        ]
    ...
    def engineer_rolling_features(self, df: pd.DataFrame, windows: List[int] = [5, 10, 20]) -> pd.DataFrame:
    ...
    def engineer_lag_features(self, df: pd.DataFrame, lags: List[int] = [1, 2, 3, 5]) -> pd.DataFrame:
        """Engineer lagged features"""
        df = df.copy()
        df = df.sort_values('timestamp')
        
        key_features = [
            'realized_vol_30m', 'atm_iv', 'rsi_30m', 'vix_level', 
    ...
    def prepare_ml_dataset(self, 
                          start_date: date,
                          end_date: date,
                          target_type: str = 'entry') -> Tuple[pd.DataFrame, List[str]]:
        ...
        exclude_cols = [
            'timestamp', 'actual_outcome', 'decision_confidence',
            'target_profitable', 'target_profit_magnitude', 'target_win_probability',
            'target_should_exit', 'target_exit_timing'
        ]
...
class DataQualityMonitor:
    ...
    def detect_feature_drift(self, 
                           ...
                           baseline_end: date,
                           current_start: date,
                           current_end: date) -> Dict[str, float]:
        """Detect feature drift between baseline and current periods"""
        session = self.session_maker()
        try:
            # Get baseline features
            baseline_features = session.query(MarketFeaturesModel).filter(
                and_(
            ...
            current_features = session.query(MarketFeaturesModel).filter(
                and_(
            ...
            key_features = [
                'atm_iv', 'realized_vol_30m', 'rsi_30m', 'vix_level',
                'bb_position', 'price_momentum_30m', 'iv_rank'
            ]
