...
class MarketMicrostructure:
    ...
    def get_avg_bid_ask_spread(self, minutes: int = 30) -> float:
        ...
        recent_spreads = [
            data['relative_spread'] for data in self.bid_ask_history 
    ...
    def get_volume_profile(self, minutes: int = 60) -> float:
        ...
        recent_volumes = [
            data['volume'] for data in self.volume_history 
            if data['timestamp'] >= cutoff_time
        ]
...
class MarketIndicatorEngine:
    ...
    def calculate_all_features(self, 
                             current_price: float,
                             implied_move: float,
                             vix_level: Optional[float] = None) -> MarketFeatures:
    ...
    def _calculate_recent_profit_factor(self) -> float:
        ...
        total_profit = sum(trade.get('profit', 0) for trade in recent_trades if trade.get('profit', 0) > 0)
        total_loss = abs(sum(trade.get('profit', 0) for trade in recent_trades if trade.get('profit', 0) < 0))
