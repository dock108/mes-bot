...
class IBClient:
    ...
    async def get_atm_straddle_price(self, underlying_price: float, expiry: str) -> Tuple[float, float, float]:
    ...
    async def place_bracket_order(self, contract: Contract, quantity: int, 
                                limit_price: float, take_profit_price: float,
                                stop_loss_price: Optional[float] = None) -> List[Trade]:
    ...
    async def place_strangle(self, underlying_price: float, call_strike: float, 
                           put_strike: float, expiry: str, max_premium: float) -> Dict:
        ...
        try:
            ...
            put_trades = await self.place_bracket_order(
                put_contract, 1, put_price, put_tp_price
            )
    ...
    def is_market_hours(self) -> bool:
        ...
        market_open = time(config.market_hours.market_open_hour, 
                          config.market_hours.market_open_minute)
        market_close = time(config.market_hours.market_close_hour, 
                           config.market_hours.market_close_minute)
