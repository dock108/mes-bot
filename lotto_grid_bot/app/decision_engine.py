...
class BaseDecisionModel(ABC):
    ...
    @abstractmethod
    async def predict_entry_signal(self, features: MarketFeatures) -> Tuple[float, Dict[str, float]]:
        """Predict entry signal strength (0-1) and feature importance"""
        pass
    
    @abstractmethod
    async def predict_exit_signal(self, features: MarketFeatures, trade_info: Dict) -> Tuple[float, Dict[str, float]]:
        """Predict exit signal strength (0-1) and reasoning"""
        pass
    
    @abstractmethod
    async def optimize_strikes(self, features: MarketFeatures, current_price: float, implied_move: float) -> Tuple[float, float]:
...
class VolatilityBasedModel(BaseDecisionModel):
    ...
    def _calculate_vol_signal(self, realized_vol: float, implied_vol: float, threshold: float) -> float:
...
class DecisionEngine:
    ...
    async def update_market_data(self, 
        ...
        if vix_level:
            self.indicator_engine.regime_detector.add_vix_data(vix_level, timestamp or datetime.utcnow())
    
    async def generate_entry_signal(self, 
                                  current_price: float, 
                                  implied_move: float,
                                  vix_level: Optional[float] = None) -> TradingSignal:
        """Generate comprehensive entry signal using all models"""
        # Calculate all market features
        features = self.indicator_engine.calculate_all_features(current_price, implied_move, vix_level)
        ...
        ensemble_signal = sum(
            model_predictions[name] * self.model_weights[name] 
        ...
        if action == "ENTER":
            try:
                ...
                optimal_strikes = await best_model.optimize_strikes(features, current_price, implied_move)
            except Exception as e:
    ...
    async def generate_exit_signal(self, 
                                 trade_info: Dict,
                                 current_price: float,
                                 implied_move: float,
                                 vix_level: Optional[float] = None) -> ExitSignal:
        ...
        ensemble_exit = sum(
            exit_predictions[name] * self.model_weights[name] 
            for name in exit_predictions
        )
        ...
        if should_exit:
            ...
            elif any("volatility_expansion" in r for r in exit_reasoning.values() if isinstance(r, dict)):
                exit_type = "MARKET_CHANGE"
                combined_reasoning.append("Volatility expansion detected")
            elif any("technical_reversal" in r for r in exit_reasoning.values() if isinstance(r, dict)):
                exit_type = "MARKET_CHANGE"
    ...
    def _generate_reasoning(self, 
                          ...
                          model_importance: Dict[str, Dict]) -> List[str]:
        ...
        if vol_ratio < 0.6:
            reasoning.append(f"Realized volatility ({vol_ratio:.1%}) significantly below implied volatility")
        
        # Technical indicators
        if 30 < features.rsi_30m < 70:
            reasoning.append(f"RSI in neutral range ({features.rsi_30m:.1f}) - good for premium selling")
        
        # Market regime
        if features.vix_level < 20:
            reasoning.append(f"Low VIX environment ({features.vix_level:.1f}) favorable for premium selling")
    ...
    def _calculate_dynamic_profit_target(self, features: MarketFeatures, confidence: float) -> float:
