...
class LottoGridBot:
    ...
    def __init__(self):
        ...
        self.strategy = LottoGridStrategy(
            self.ib_client, 
            self.risk_manager, 
            config.database.url
        )
    ...
    async def _daily_summary(self):
        ...
        try:
            ...
            self._log_system_event("DAILY_SUMMARY", "Daily summary generated and reset for next day")
    ...
    def _log_system_event(self, event_type: str, message: str, details: Optional[str] = None, level: str = 'INFO'):
    ...
    def _handle_high_error_rate(self):
        ...
        logger.critical(f"High error rate detected: {len(self.error_timestamps)} errors in last hour")
        self.trading_halted = True
    ...
    def get_health_status(self) -> Dict:
        ...
        # Check IB connection
        health_status['ib_connection_status'] = 'healthy' if hasattr(self.ib_client, 'is_connected') and self.ib_client.is_connected() else 'unknown'
