...
from app.decision_engine import (
    DecisionEngine, TradingSignal, ExitSignal, DecisionConfidence,
    VolatilityBasedModel, MLEnsembleModel, BaseDecisionModel
)
...
class TestVolatilityBasedModel:
    ...
    async def test_entry_signal_high_volatility(self, model):
        ...
        high_vol_features = MarketFeatures(
            ...
            iv_rank=50.0,
            iv_percentile=50.0,
            iv_skew=0.0, iv_term_structure=0.0,
            rsi_15m=50.0, rsi_30m=50.0,
            macd_signal=0.0, macd_histogram=0.0,
            bb_position=0.5, bb_squeeze=0.0,
            price_momentum_15m=0.0, price_momentum_30m=0.0, price_momentum_60m=0.0,
            support_resistance_strength=0.0, mean_reversion_signal=0.0,
            bid_ask_spread=0.0, option_volume_ratio=0.0, put_call_ratio=1.0,
            gamma_exposure=0.0, vix_level=25.0, vix_term_structure=0.0,
            market_correlation=0.0, volume_profile=1.0,
            time_of_day=12.0, day_of_week=2.0, time_to_expiry=4.0,
            days_since_last_trade=0.0, win_rate_recent=0.25,
            profit_factor_recent=1.0, sharpe_ratio_recent=0.0,
            timestamp=datetime.utcnow()
        )
    ...
    async def test_entry_signal_favorable_conditions(self, model):
        ...
        favorable_features = MarketFeatures(
            ...
            iv_percentile=20.0,
            iv_skew=0.0, iv_term_structure=0.0,
            rsi_15m=45.0, rsi_30m=50.0,  # Neutral RSI
            macd_signal=0.0, macd_histogram=0.0,
            bb_position=0.4, bb_squeeze=0.01,  # Squeeze condition
            price_momentum_15m=0.0, price_momentum_30m=0.0, price_momentum_60m=0.0,
            support_resistance_strength=0.0, mean_reversion_signal=0.0,
            bid_ask_spread=0.005, option_volume_ratio=0.0, put_call_ratio=1.0,
            gamma_exposure=0.0, vix_level=16.0,  # Low VIX
            vix_term_structure=0.0, market_correlation=0.0, volume_profile=1.0,
            time_of_day=13.0, day_of_week=2.0,  # Good time
            time_to_expiry=4.0, days_since_last_trade=0.0,
            win_rate_recent=0.25, profit_factor_recent=1.0, sharpe_ratio_recent=0.0,
            timestamp=datetime.utcnow()
        )
    ...
    async def test_exit_signal_time_decay(self, model, sample_features):
        ...
        near_expiry_features = MarketFeatures(**{
            **asdict(sample_features),
            'time_to_expiry': 1.0  # 1 hour to expiry
        })
    ...
    async def test_strike_optimization_overbought(self, model):
        ...
        overbought_features = MarketFeatures(
            realized_vol_15m=0.1, realized_vol_30m=0.1, realized_vol_60m=0.1,
            realized_vol_2h=0.1, realized_vol_daily=0.1,
            atm_iv=0.2, iv_rank=50.0, iv_percentile=50.0,
            iv_skew=0.0, iv_term_structure=0.0,
            rsi_15m=75.0, rsi_30m=80.0,  # Overbought
            macd_signal=0.0, macd_histogram=0.0,
            bb_position=0.5, bb_squeeze=0.0,
            price_momentum_15m=0.0, price_momentum_30m=0.0, price_momentum_60m=0.0,
            support_resistance_strength=0.0, mean_reversion_signal=0.0,
            bid_ask_spread=0.0, option_volume_ratio=0.0, put_call_ratio=1.0,
            gamma_exposure=0.0, vix_level=20.0, vix_term_structure=0.0,
            market_correlation=0.0, volume_profile=1.0,
            time_of_day=12.0, day_of_week=2.0, time_to_expiry=4.0,
            days_since_last_trade=0.0, win_rate_recent=0.25,
            profit_factor_recent=1.0, sharpe_ratio_recent=0.0,
...
class TestMLEnsembleModel:
    ...
    def sample_features(self):
        ...
        return MarketFeatures(
            ...
            market_correlation=0.75, volume_profile=1.15,
            time_of_day=14.0, day_of_week=3.0, time_to_expiry=3.5,
            days_since_last_trade=1.5, win_rate_recent=0.32,
            profit_factor_recent=1.9, sharpe_ratio_recent=1.3,
            timestamp=datetime.utcnow()
        )
...
class TestDecisionEngine:
    ...
    async def test_entry_signal_generation(self, engine):
        ...
        with patch.object(engine.indicator_engine, 'calculate_all_features') as mock_calc:
            test_features = MarketFeatures(
                realized_vol_15m=0.10, realized_vol_30m=0.12, realized_vol_60m=0.15,
                realized_vol_2h=0.18, realized_vol_daily=0.20,
                ...
                time_of_day=12.5, day_of_week=2.0, time_to_expiry=4.0,
                days_since_last_trade=2.0, win_rate_recent=0.30,
                profit_factor_recent=1.8, sharpe_ratio_recent=1.1,
                timestamp=datetime.utcnow()
            )
    ...
    async def test_entry_signal_high_confidence(self, engine):
        ...
        with patch.object(engine.indicator_engine, 'calculate_all_features') as mock_calc:
            ...
            favorable_features = MarketFeatures(
                realized_vol_15m=0.08, realized_vol_30m=0.10, realized_vol_60m=0.12,
                realized_vol_2h=0.14, realized_vol_daily=0.16,
                atm_iv=0.30, iv_rank=20.0, iv_percentile=15.0,  # High IV, low rank
                iv_skew=0.01, iv_term_structure=0.005,
                rsi_15m=48.0, rsi_30m=52.0, macd_signal=0.0, macd_histogram=0.0,
                bb_position=0.45, bb_squeeze=0.008,  # Tight squeeze
                price_momentum_15m=0.002, price_momentum_30m=0.003, price_momentum_60m=0.005,
                support_resistance_strength=0.1, mean_reversion_signal=0.05,
                bid_ask_spread=0.001, option_volume_ratio=1.0, put_call_ratio=1.0,
                gamma_exposure=1000.0, vix_level=15.0,  # Low VIX
                vix_term_structure=0.01, market_correlation=0.8, volume_profile=0.95,
                time_of_day=13.0, day_of_week=2.0, time_to_expiry=4.5,  # Good timing
                days_since_last_trade=3.0, win_rate_recent=0.35,
                profit_factor_recent=2.2, sharpe_ratio_recent=1.5,
                timestamp=datetime.utcnow()
            )
    ...
    def test_reasoning_generation(self, engine):
        ...
        assert any(keyword in reasoning_text.lower() for keyword in 
                  ["volatility", "rsi", "vix", "time", "agreement"])
    ...
    def test_key_features_extraction(self, engine, mock_features):
        ...
        expected_keys = [
            'realized_vol_30m', 'atm_iv', 'iv_rank', 'rsi_30m',
            'vix_level', 'time_of_day', 'time_to_expiry', 'bb_position'
        ]
    ...
    def test_dynamic_profit_target_time_adjustment(self, engine):
        ...
        near_expiry_features = MarketFeatures(
            ...
            iv_skew=0.0, iv_term_structure=0.0,
            rsi_15m=50.0, rsi_30m=50.0, macd_signal=0.0, macd_histogram=0.0,
            bb_position=0.5, bb_squeeze=0.0,
            price_momentum_15m=0.0, price_momentum_30m=0.0, price_momentum_60m=0.0,
            support_resistance_strength=0.0, mean_reversion_signal=0.0,
            bid_ask_spread=0.0, option_volume_ratio=0.0, put_call_ratio=1.0,
            gamma_exposure=0.0, vix_level=20.0, vix_term_structure=0.0,
            market_correlation=0.0, volume_profile=1.0,
            time_of_day=15.0, day_of_week=3.0, 
        ...
        far_expiry_features = MarketFeatures(**{
            **asdict(near_expiry_features),
            'time_to_expiry': 6.0  # Far from expiry
        })
...
class TestDecisionEngineIntegration:
    ...
    async def test_complete_decision_workflow(self, engine):
        ...
        if entry_signal.action == "ENTER":
            trade_info = {
                ...
                'call_strike': entry_signal.optimal_strikes[0] if entry_signal.optimal_strikes else base_price + 25,
                'put_strike': entry_signal.optimal_strikes[1] if entry_signal.optimal_strikes else base_price - 25,
                'current_pnl': 25.0,
                'time_in_trade': 0.5
            }
    ...
    async def test_model_ensemble_agreement(self, engine):
        ...
        signal = await engine.generate_entry_signal(
            current_price=4200.0,
            implied_move=30.0,  # High implied move
            vix_level=15.0
        )
