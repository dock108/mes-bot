...
from app.ml_training import (
    BaseMLModel, EntryPredictionModel, ExitPredictionModel, StrikeOptimizationModel,
    ModelTrainer, ModelScheduler, TrainingConfig, ModelPerformance
)
...
class TestEntryPredictionModel:
    ...
    def sample_training_data(self):
        ...
        target_probs = (features['iv_rank'] / 100 * 0.3 + 
    ...
    def test_entry_model_prepare_targets(self, training_config):
        ...
        df_with_target = pd.DataFrame({
            'target_profitable': [1, 0, 1, 0, 1],
            'actual_outcome': [100, -50, 75, -25, 120]
        })
...
class TestExitPredictionModel:
    ...
    def exit_training_data(self):
        ...
        exit_signals = ((features['time_to_expiry'] < 1.5) | 
                       (features['current_pnl_pct'] > 1.5)).astype(int)
...
class TestStrikeOptimizationModel:
    ...
    def strike_training_data(self):
        ...
        profits = (features['iv_rank'] / 100 * 100 + 
...
class TestModelTrainer:
    ...
    async def test_train_entry_model_success(self, trainer, mock_feature_engineer):
        ...
        trainer.data_quality_monitor.check_data_quality = Mock(return_value={
            'completeness': 0.9,
            'consistency': 0.85,
            'freshness': 0.8
        })
    ...
    @pytest.mark.asyncio
    async def test_train_entry_model_insufficient_data_quality(self, trainer, mock_feature_engineer):
    ...
    async def test_save_model_metadata(self, trainer):
        ...
        try:
            metadata = session.query(MLModelMetadata).filter(
                MLModelMetadata.model_name == 'test_entry_model'
            ).first()
...
class TestMLTrainingIntegration:
    ...
    async def test_complete_training_workflow(self, full_trainer):
        ...
        # Mock data quality check
        full_trainer.data_quality_monitor.check_data_quality = Mock(return_value={
            'completeness': 0.9,
            'consistency': 0.85,
            'freshness': 0.8
        })
        ...
        try:
            metadata = session.query(MLModelMetadata).filter(
                MLModelMetadata.model_type == 'entry'
            ).first()
    ...
    async def test_model_versioning(self, full_trainer):
        ...
        session = full_trainer.session_maker()
        try:
            active_models = session.query(MLModelMetadata).filter(
                MLModelMetadata.model_type == 'entry',
