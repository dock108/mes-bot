...
from app.models import (
    ...
    DecisionHistory, MLModelMetadata, MLPrediction, Trade, 
    get_session_maker
)
...
class TestMLPipelineEndToEnd:
    ...
    @pytest.mark.asyncio
    async def test_complete_ml_pipeline_workflow(self, feature_collector, feature_engineer, 
                                                 model_trainer, decision_engine, session_maker):
    ...
    def _create_synthetic_training_data(self):
        ...
        profit_signal = (
            (df['iv_rank'] / 100) * 0.3 +  # High IV rank is good
            ((0.30 - df['realized_vol_30m']) / 0.20) * 0.3 +  # Low realized vol is good
            ((df['atm_iv'] - df['realized_vol_30m']) / 0.10) * 0.2 +  # IV > realized vol
            np.random.uniform(0, 0.2, n_samples)  # Some randomness
        )
    ...
    async def _generate_ml_predictions(self, models, feature_engineer, session_maker):
        ...
        try:
            ...
            recent_features = session.query(MarketFeaturesModel).order_by(
                MarketFeaturesModel.timestamp.desc()
            ).first()
            ...
            if not recent_features:
                ...
                recent_features = MarketFeaturesModel(
                    ...
                    realized_vol_15m=0.12, realized_vol_30m=0.15,
                    atm_iv=0.25, iv_rank=70.0,
                    rsi_15m=45.0, rsi_30m=50.0,
                    vix_level=18.0, time_of_day=13.0,
                    day_of_week=2.0, time_to_expiry=4.0
                )
    ...
    async def _make_trading_decision(self, decision_engine, predictions):
        ...
        mock_features = MarketFeatures(
            ...
            time_of_day=13.0, day_of_week=2.0, time_to_expiry=4.0,
...
class TestMLPipelineRobustness:
    ...
    async def test_pipeline_with_insufficient_data(self, database_url):
        ...
        await feature_collector.collect_market_data(
            price=4200.0, bid=4199.5, ask=4200.5,
            volume=1000, atm_iv=0.25, vix_level=20.0
        )
        ...
        try:
            ...
            try:
                ...
                result = feature_engineer.create_features_for_prediction(
                    MarketFeatures(
                        realized_vol_15m=999.0, realized_vol_30m=999.0, realized_vol_60m=999.0,
                        realized_vol_2h=999.0, realized_vol_daily=999.0,
                        atm_iv=-1.0, iv_rank=150.0, iv_percentile=150.0,
                        iv_skew=999.0, iv_term_structure=999.0,
                        rsi_15m=-50.0, rsi_30m=-50.0, macd_signal=999.0, macd_histogram=999.0,
                        bb_position=999.0, bb_squeeze=999.0,
                        price_momentum_15m=999.0, price_momentum_30m=999.0, price_momentum_60m=999.0,
                        support_resistance_strength=999.0, mean_reversion_signal=999.0,
                        bid_ask_spread=999.0, option_volume_ratio=999.0, put_call_ratio=999.0,
                        gamma_exposure=999.0, vix_level=1000.0, vix_term_structure=999.0,
                        market_correlation=999.0, volume_profile=999.0,
                        time_of_day=25.0, day_of_week=8.0, time_to_expiry=-1.0,
                        days_since_last_trade=-1.0, win_rate_recent=999.0,
                        profit_factor_recent=999.0, sharpe_ratio_recent=999.0,
                        timestamp=datetime.utcnow()
                    )
                )
                # If it succeeds, the pipeline should handle invalid values gracefully
                assert isinstance(result, np.ndarray), "Should return valid array even with bad input"
            except Exception as e:
...
class TestMLPipelinePerformance:
    ...
    async def test_feature_collection_performance(self, database_url):
        ...
        for i in range(100):  # 100 data points
            await feature_collector.collect_market_data(
                ...
                bid=4199.5, ask=4200.5,
                volume=1000, atm_iv=0.25,
                vix_level=20.0,
    ...
    async def test_feature_engineering_batch_performance(self, database_url):
        ...
        try:
            ...
            assert engineering_time < 30.0, f"Feature engineering took {engineering_time:.2f}s, too slow"
...
class TestMLPipelineIntegrationWithStrategy:
    ...
    @pytest.mark.asyncio
    async def test_enhanced_strategy_with_ml_pipeline(self, database_url, mock_ib_client, mock_risk_manager):
        ...
        with patch.object(strategy.__class__.__bases__[0], 'initialize_daily_session', new_callable=AsyncMock) as mock_parent_init:
