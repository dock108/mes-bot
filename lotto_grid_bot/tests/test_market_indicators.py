...
from app.market_indicators import (
    TechnicalIndicators, VolatilityAnalyzer, MarketMicrostructure,
    MarketRegimeDetector, MarketIndicatorEngine, MarketFeatures
)
...
class TestTechnicalIndicators:
    ...
    def test_rsi_calculation(self):
        ...
        prices = [44, 44.34, 44.09, 44.15, 43.61, 44.33, 44.83, 45.85, 46.08, 45.89, 
                 46.03, 46.83, 47.69, 46.49, 46.26, 47.09, 47.37, 47.20, 47.72, 48.90]
...
class TestMarketMicrostructure:
    ...
    def test_market_data_addition(self, microstructure):
        ...
        now = datetime.utcnow()
        microstructure.add_market_data(
            bid=99.5, ask=100.5, volume=1000, timestamp=now
        )
...
class TestMarketFeaturesDataclass:
    ...
    def test_market_features_validation(self):
        ...
        features = MarketFeatures(
            realized_vol_15m=0.15, realized_vol_30m=0.18, realized_vol_60m=0.20,
            realized_vol_2h=0.22, realized_vol_daily=0.25, atm_iv=0.23,
            iv_rank=65.0, iv_percentile=70.0, iv_skew=0.05, iv_term_structure=0.02,
            rsi_15m=55.0, rsi_30m=58.0, macd_signal=0.15, macd_histogram=0.05,
            bb_position=0.6, bb_squeeze=0.02, price_momentum_15m=0.01,
            price_momentum_30m=0.015, price_momentum_60m=0.02,
            support_resistance_strength=0.3, mean_reversion_signal=0.1,
            bid_ask_spread=0.005, option_volume_ratio=1.2, put_call_ratio=0.9,
            gamma_exposure=1500.0, vix_level=22.0, vix_term_structure=0.03,
            market_correlation=0.75, volume_profile=1.1, time_of_day=14.5,
            day_of_week=2.0, time_to_expiry=3.5, days_since_last_trade=1.0,
            win_rate_recent=0.28, profit_factor_recent=1.8, sharpe_ratio_recent=1.2,
            timestamp=datetime.utcnow()
        )
...
class TestIndicatorEngineIntegration:
    ...
    def test_feature_consistency_across_timeframes(self, engine):
        ...
        features = engine.calculate_all_features(
            current_price=base_price + 60,  # Final price
            implied_move=25.0,
            vix_level=20.0
        )
