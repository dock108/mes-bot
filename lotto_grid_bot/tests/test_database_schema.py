...
from app.models import (
    Base, Trade, DailySummary, BacktestResult, SystemLog,
    MarketData, MarketFeatures, DecisionHistory, MLModelMetadata,
    MLPrediction, PerformanceMetrics, create_database, get_session_maker
)
...
class TestDatabaseSchema:
    ...
    def test_database_creation(self, engine):
        ...
        expected_tables = [
            'trades', 'daily_summaries', 'backtest_results', 'system_logs',
            'market_data', 'market_features', 'decision_history',
            'ml_model_metadata', 'ml_predictions', 'performance_metrics'
        ]
    ...
    def test_table_columns(self, engine):
        ...
        market_data_columns = {col['name']: col['type'] for col in inspector.get_columns('market_data')}
        expected_market_data_cols = [
            'id', 'timestamp', 'underlying_price', 'bid_price', 'ask_price',
            'volume', 'atm_iv', 'implied_move', 'vix_level'
        ]
        for col in expected_market_data_cols:
            assert col in market_data_columns
        
        # Test MarketFeatures columns
        features_columns = {col['name']: col['type'] for col in inspector.get_columns('market_features')}
        expected_features_cols = [
            'id', 'timestamp', 'realized_vol_15m', 'realized_vol_30m', 'atm_iv',
            'iv_rank', 'rsi_15m', 'rsi_30m', 'bb_position', 'vix_level'
        ]
        for col in expected_features_cols:
            assert col in features_columns
        
        # Test MLModelMetadata columns
        ml_metadata_columns = {col['name']: col['type'] for col in inspector.get_columns('ml_model_metadata')}
        expected_ml_cols = [
            'id', 'model_name', 'model_type', 'version', 'trained_on',
            'hyperparameters', 'validation_accuracy', 'is_active'
        ]
...
class TestMarketFeaturesModel:
    ...
    def test_market_features_all_fields(self, session):
        ...
        features = MarketFeatures(
            ...
            # Market regime
            vix_level=19.0, vix_term_structure=0.025, market_correlation=0.75,
            volume_profile=1.15,
            # Time features
...
class TestMLPredictionModel:
    ...
    def test_ml_prediction_relationships(self, session):
        ...
        trade = Trade(
            underlying_price_at_entry=4200.0, implied_move=25.0,
            call_strike=4225.0, put_strike=4175.0,
            call_premium=15.0, put_premium=12.0, total_premium=27.0
        )
        decision = DecisionHistory(
            action='ENTER', confidence=0.8,
            underlying_price=4200.0, implied_move=25.0
        )
...
class TestDatabaseRelationships:
    ...
    def test_ml_prediction_relationships(self, session):
        ...
        prediction = MLPrediction(
            model_id=model.id, model_name='test_model',
            prediction_type='entry', prediction_value=0.7, confidence=0.8,
            features_id=features.id
        )
        session.add(prediction)
        session.commit()
...
class TestDatabasePerformance:
    ...
    def test_query_performance_with_indexes(self, session):
        ...
        results = session.query(MarketData).filter(
            ...
            MarketData.timestamp <= query_end
        ).all()
...
class TestDatabaseMigrationSupport:
    ...
    def test_schema_introspection(self, database_url):
        ...
        expected_tables = [
            'trades', 'market_data', 'market_features', 'decision_history',
            'ml_model_metadata', 'ml_predictions', 'performance_metrics'
        ]
