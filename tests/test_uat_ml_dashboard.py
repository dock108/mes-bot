    ...
    def test_trading_performance_visualization(self, sample_data):
        ...
        try:
            ...
            def create_performance_chart():
                ...
                for trade in trades:
                    trade_data.append(
                        {"Date": trade.date, "P&L": trade.realized_pnl or 0, "Status": trade.status}
                    )
            ...
            if len(chart_data) > 0:
                assert (
                    chart_data["Cumulative P&L"].iloc[-1] == chart_data["P&L"].sum()
                ), "Cumulative calculation should be correct"
    ...
    def test_ml_model_accuracy_tracking(self, ml_performance_data):
        ...
        try:
            ...
            def calculate_accuracy_over_time():
                ...
                if len(df) == 0:
                    return pd.DataFrame(
                        columns=["timestamp", "correct", "confidence", "rolling_accuracy"]
                    )
                ...
                window_size = 20
                df["rolling_accuracy"] = (
                    df["correct"].rolling(window=window_size, min_periods=1).mean()
                )
        ...
        try:
            ...
            def analyze_prediction_confidence():
                ...
                accuracy_by_confidence = (
                    df.groupby("confidence_bucket")["correct"].agg(["mean", "count"]).reset_index()
                )
            ...
            assert (
                "accuracy" in confidence_analysis.columns
            ), "Should calculate accuracy by confidence"
            ...
            high_conf_accuracy = confidence_analysis[
                confidence_analysis["confidence_bucket"].isin(["High", "Very High", "Extreme"])
            ]["accuracy"].mean()
            low_conf_accuracy = confidence_analysis[
                confidence_analysis["confidence_bucket"] == "Low"
            ]["accuracy"].mean()
...
class TestRealTimeDataDisplay:
    ...
    def test_live_decision_monitoring(self, database_url):
        ...
        assert all(
            "%" in conf for conf in formatted_decisions["Confidence"]
...
class TestUserInteractionWorkflows:
    ...
    def test_alert_and_notification_system(self, database_url):
        ...
        assert any(
            alert["type"] == "model_performance" for alert in alerts
        ), "Should have model performance alert"
        ...
        assert all(
            "🔴" in sev or "🟡" in sev or "🔵" in sev for sev in formatted_alerts["Severity"]
        ...
        def validate_control_settings(settings):
            ...
            if settings["confidence_threshold"] > 0.9:
                validation["warnings"].append(
                    "High confidence threshold may reduce trading frequency"
                )
...
class TestDashboardAccessibilityAndUsability:
    ...
    def test_error_handling_and_user_feedback(self):
        ...
        assert all(
            "user_message" in error for error in error_scenarios
        ), "All errors should have user messages"
        assert any(
            error["severity"] == "critical" for error in error_scenarios
        ), "Should have critical errors"

        # Test user-friendly error messages
        for error in error_scenarios:
            assert len(error["user_message"]) > 10, "User messages should be descriptive"
            assert not error["user_message"].startswith(
                "Traceback"
...
if __name__ == "__main__":
    # Run UAT tests
    pytest.main([__file__, "-v", "-s", "--tb=short", "-m", "uat"])
