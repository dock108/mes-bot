...
class TestFeatureCollector:
    ...
    async def test_collect_market_data_indicator_engine_update(self, collector):
        ...
        await collector.collect_market_data(
            price=4200.0, bid=4199.5, ask=4200.5, volume=1000, atm_iv=0.25, timestamp=timestamp
        )
    ...
    async def test_calculate_and_store_features(self, collector):
        ...
        features_id = await collector.calculate_and_store_features(
            current_price=4230.0, implied_move=25.0, vix_level=20.0, timestamp=timestamp
        )
        ...
        try:
            features = (
                ...
                .filter(MarketFeaturesModel.id == features_id)
                .first()
            )
    ...
    async def test_calculate_and_store_features_insufficient_data(self, collector):
        ...
        features_id = await collector.calculate_and_store_features(
            current_price=4200.0, implied_move=25.0, vix_level=20.0, timestamp=timestamp
        )
...
class TestFeatureEngineer:
    ...
    def test_engineer_interaction_features(self, engineer):
        ...
        assert np.allclose(
            enhanced_df["vol_ratio_15_30"], df["realized_vol_15m"] / (df["realized_vol_30m"] + 1e-8)
        )
        assert np.allclose(enhanced_df["rsi_divergence"], df["rsi_15m"] - df["rsi_30m"])
    ...
    def test_engineer_rolling_features(self, engineer):
        ...
        df = pd.DataFrame(
            {
                "timestamp": [
                    datetime.utcnow() - timedelta(hours=n_points - i) for i in range(n_points)
                ],
...
class TestFeaturePipelineIntegration:
    ...
    async def test_complete_pipeline_workflow(self):
        ...
        quality_metrics = monitor.check_data_quality(start_time.date(), datetime.utcnow().date())
        ...
        df, feature_cols = engineer.prepare_ml_dataset(
            start_time.date(), datetime.utcnow().date(), target_type="entry"
        )
    ...
    def test_pipeline_performance_metrics(self, pipeline_components):
        ...
        features_id = asyncio.run(
            collector.calculate_and_store_features(
