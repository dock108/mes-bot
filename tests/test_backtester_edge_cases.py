...
class TestBacktesterEdgeCases:
    ...
    def test_calculate_implied_volatility_insufficient_data(self, backtester):
        ...
        mock_data = pd.DataFrame({
            "Close": [4250]
        }, index=pd.date_range(start="2025-07-10", periods=1, freq="5min"))
    ...
    def test_calculate_atm_straddle_price_edge_values(self, backtester):
        ...
        call_price, put_price, total_price = backtester.calculate_atm_straddle_price(4250, 0.0833, 0.01)
        assert total_price > 0
        
        # Very high volatility
        call_price, put_price, total_price = backtester.calculate_atm_straddle_price(4250, 0.0833, 2.0)
        assert total_price > 0
        
        # Zero time to expiry
        call_price, put_price, total_price = backtester.calculate_atm_straddle_price(4250, 0.0, 0.20)
        assert total_price == 0
    ...
    def test_update_trade_pnl_at_expiry(self, backtester):
        ...
        with patch.object(backtester, 'bs_calculator') as mock_bs:
            ...
            result = backtester.update_trade_pnl(mock_trade, current_price, time_to_expiry, volatility)
