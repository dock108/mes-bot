...
class TestEnhancedLottoGridStrategy:
    ...
    def mock_ib_client(self):
        ...
        client.place_strangle = AsyncMock(return_value={
            'call_strike': 4225.0,
            'put_strike': 4175.0,
            'call_price': 15.0,
            'put_price': 12.0,
    ...
    def enhanced_strategy(self, mock_ib_client, mock_risk_manager, database_url):
        ...
        strategy = EnhancedLottoGridStrategy(
            mock_ib_client, 
            mock_risk_manager, 
            database_url
        )
    ...
    @pytest.mark.asyncio
    async def test_enhanced_initialization_ml_failure_fallback(self, enhanced_strategy, mock_ib_client):
        ...
        enhanced_strategy._initialize_decision_engine = AsyncMock(side_effect=Exception("ML init failed"))
    ...
    async def test_should_place_trade_enhanced_ml_enabled(self, enhanced_strategy):
        ...
        with patch.object(enhanced_strategy, 'should_place_trade', return_value=(True, "Basic conditions met")):
            ...
            enhanced_strategy._collect_market_features = AsyncMock()
            enhanced_strategy.decision_engine.generate_entry_signal = AsyncMock(return_value=ml_signal)
    ...
    async def test_should_place_trade_enhanced_ml_error_fallback(self, enhanced_strategy):
        ...
        with patch.object(enhanced_strategy, 'should_place_trade', return_value=(True, "Basic conditions met")):
            # Mock ML error
            enhanced_strategy._collect_market_features = AsyncMock(side_effect=Exception("ML error"))
    ...
    @pytest.mark.asyncio
    async def test_execute_trading_cycle_enhanced_with_ml_strikes(self, enhanced_strategy, mock_ib_client):
        ...
        enhanced_strategy.should_place_trade_enhanced = AsyncMock(return_value=(True, "Good conditions", ml_signal))
    ...
    @pytest.mark.asyncio
    async def test_execute_trading_cycle_enhanced_fallback_strikes(self, enhanced_strategy, mock_ib_client):
    ...
    @pytest.mark.asyncio
    async def test_execute_trading_cycle_enhanced_ml_failure_fallback(self, enhanced_strategy, mock_ib_client):
        ...
        with patch.object(LottoGridStrategy, 'execute_trading_cycle', return_value=True) as mock_basic:
            result = await enhanced_strategy.execute_trading_cycle_enhanced()
    ...
    @pytest.mark.asyncio
    async def test_place_enhanced_strangle_trade(self, enhanced_strategy, mock_ib_client, mock_risk_manager):
    ...
    @pytest.mark.asyncio
    async def test_place_enhanced_strangle_trade_risk_check_failure(self, enhanced_strategy, mock_ib_client, mock_risk_manager):
    ...
    async def test_collect_market_features(self, enhanced_strategy):
        ...
        enhanced_strategy.feature_collector.calculate_and_store_features = AsyncMock(return_value=123)
        enhanced_strategy._get_current_vix = AsyncMock(return_value=20.0)
...
class TestEnhancedStrategyIntegration:
    ...
    async def test_complete_enhanced_workflow(self, integration_strategy):
        ...
        # Initialize strategy
        with patch.object(integration_strategy, '_initialize_decision_engine'), \
        ...
        with patch.object(integration_strategy.decision_engine, 'generate_entry_signal') as mock_signal:
    ...
    async def test_error_resilience(self, integration_strategy):
        ...
        with patch.object(integration_strategy, '_initialize_decision_engine', side_effect=Exception("ML error")), \
        ...
        with patch.object(integration_strategy, 'should_place_trade', return_value=(True, "Basic OK")):
            should_trade, reason, signal = await integration_strategy.should_place_trade_enhanced()
            assert should_trade is True
    ...
    async def test_performance_tracking(self, integration_strategy):
        ...
        signal1 = TradingSignal(
            action="ENTER", confidence=0.8, reasoning=[], 
            features_used={}, model_predictions={}
        )
        signal2 = TradingSignal(
            action="HOLD", confidence=0.3, reasoning=[],
            features_used={}, model_predictions={}
        )
