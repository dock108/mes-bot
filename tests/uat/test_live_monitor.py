...
class TestLiveMonitoring:
    ...
    def test_open_positions_display(self, dashboard_with_data):
        ...
        if positions_count > 0:
            ...
            if position_data:
                ...
                assert has_trading_fields, f"Missing trading fields in position data: {first_position.keys()}"
    ...
    def test_position_metrics_accuracy(self, dashboard_with_data):
        ...
        if "Open Positions" in metrics:
            ...
            try:
                ...
                assert metric_count == positions_count, \
    ...
    def test_live_data_refresh_cycle(self, dashboard_with_data):
        ...
        assert len(initial_positions) == len(updated_positions), \
    ...
    def test_live_pnl_calculations(self, dashboard_with_data):
        ...
        if pnl_metrics:
            ...
            for metric_name, metric_value in pnl_metrics.items():
                ...
                assert has_currency or has_number, \
        ...
        if position_data:
            for position in position_data:
                pnl_fields = [k for k in position.keys() if "p&l" in k.lower() or "pnl" in k.lower()]
                
                for field in pnl_fields:
                    value = position[field]
                    # Should be a valid P&L format
                    if value and value != "N/A":
                        has_valid_format = "$" in value or "%" in value or any(char.isdigit() for char in value)
