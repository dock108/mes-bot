...
class DashboardPage(BasePage):
    ...
    def get_current_section(self) -> str:
        ...
        for i in range(radio_labels.count()):
            ...
            if label.get_attribute("data-checked") == "true" or "selected" in (
        ...
        for selector in possible_selectors:
            ...
            if metric_containers.count() > 0:
                for i in range(metric_containers.count()):
                    ...
                    label_text = container.locator(
                        'label, .metric-label, [data-testid="stMetricLabel"]'
                    ).text_content()
                    value_text = container.locator(
    ...
    def set_trading_mode(self, mode: str):
        ...
        mode_button = (
            self.main_content.locator(f'label:has-text("{mode}")')
            .locator("..")
    ...
    def get_entry_conditions(self) -> Dict[str, bool]:
        ...
        condition_elements = self.main_content.locator(
            "text=/RV < 67% of IV|Time between trades OK|Risk limits OK|Premium in range/"
        )
...
class BacktestPage(BasePage):
    ...
    def wait_for_backtest_completion(self, timeout: int = 60000):
        ...
        self.page.wait_for_selector(
            'text="Backtest Complete" | [data-testid="stPlotlyChart"]', timeout=timeout
        )
    ...
    def set_date_input(self, label: str, date_value: str):
        ...
        date_input = (
            self.main_content.locator(f'label:has-text("{label}")')
            .locator("..")
