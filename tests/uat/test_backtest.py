...
class TestBacktestingWorkflow:
    ...
    def test_backtest_execution_workflow(self, dashboard_with_data):
        ...
        run_buttons = page.locator('button:has-text("Run"), button:has-text("Start"), button:has-text("Execute")')
    ...
    def test_backtest_results_display(self, dashboard_with_data):
        ...
        if results:
            ...
            expected_result_metrics = [
                "Total Return", "Win Rate", "Total Trades", "Max Drawdown",
                "Sharpe Ratio", "Profit Factor", "Final Capital"
            ]
            ...
            for metric_name, metric_value in results.items():
                ...
                if "rate" in metric_lower or "%" in metric_name:
                    has_valid_format = "%" in metric_value or any(char.isdigit() for char in metric_value)
                    assert has_valid_format, f"Rate metric '{metric_name}' invalid format: '{metric_value}'"
                
                elif "return" in metric_lower or "capital" in metric_lower or "drawdown" in metric_lower:
                    has_valid_format = "$" in metric_value or "%" in metric_value or any(char.isdigit() for char in metric_value)
                    assert has_valid_format, f"Financial metric '{metric_name}' invalid format: '{metric_value}'"
        ...
        for param_name, invalid_value in invalid_parameters.items():
            try:
                ...
                if error_messages.count() > 0:
                    ...
                    dashboard.take_screenshot(f"validation_error_{param_name.lower().replace(' ', '_')}")
    ...
    def test_backtest_date_validation(self, dashboard_with_data):
        ...
        start_date_input = page.locator('label:has-text("Start Date")').locator('..').locator('input[type="date"]')
        end_date_input = page.locator('label:has-text("End Date")').locator('..').locator('input[type="date"]')
    ...
    def test_date_range_validation(self, dashboard_with_data):
        ...
        if run_button.count() > 0:
            ...
            # Should show error
            error_message = page.locator('text=/Start date must be before end date|Invalid date range/')
            if error_message.count() > 0:
    ...
    def test_backtest_comparison_functionality(self, dashboard_with_data):
        ...
        if compare_elements.count() > 0:
            ...
            comparison_viz = page.locator('[data-testid="stPlotlyChart"], [data-testid="stDataFrame"]')
    ...
    def test_backtest_export_functionality(self, dashboard_with_data):
        ...
        export_elements = page.locator('button:has-text("Export"), button:has-text("Download"), text=/CSV|Excel/')
        ...
        if export_elements.count() > 0:
            ...
            for i in range(min(2, export_elements.count())):  # Test first 2 options
                ...
                if element_text and any(keyword in element_text.lower() for keyword in ["export", "download"]):
